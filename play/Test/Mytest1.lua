...
local AstandPos = function()
		...
		debugEngine:gui_debug_msg(CGeoPoint:new_local(0,0),tostring(isLeaveBallPlace("a", 500)))
...
["state2"] = { --转向
	switch = function()
		...
		debugEngine:gui_debug_line(testPlacePos1(),testBallPos1(),1)
		debugEngine:gui_debug_line(testPlacePos2(),testBallPos2(),1)
		if  math.abs((testPlacePos - player.pos("Leader")):dir() - player.dir("Leader"))  < 0.04 then
			return "state3"
		end
	end,
...
["state3"] = { --带球移动到目标位置
	switch = function()
		...
		if bufcnt(player.toBallDist("Leader") > 1000, 10) then
			return "state0"
		-- elseif bufcnt(player.toTargetDist("Leader" < 100, 10)) then
		-- 	-- return "state4"
		end
	end,

	Leader = task.goCmuRush(PtestPlacePos("Leader"), player.toTargetDir(testPlacePos, "Leader"),_,flag.dribbling + flag.our_ball_placement),
	a      = task.goCmuRush(AstandPos(), dir.playerToBall,_,DSS_FLAG),
	b      = task.goCmuRush(BstandPos, dir.playerToBall,_,DSS_FLAG),
	c      = task.goCmuRush(CstandPos, dir.playerToBall,_,DSS_FLAG),
	d      = task.goCmuRush(DstandPos, dir.playerToBall,_,DSS_FLAG),
